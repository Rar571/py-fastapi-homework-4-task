...
async def register_user(
    ...
    try:
        ...
        background_tasks.add_task(
            email_sender.send_activation_email, str(user_data.email), activation_url
        )
...
async def activate_account(
    ...
    login_url = "http://127.0.0.1/accounts/login/"
    background_tasks.add_task(
...
async def request_password_reset_token(
    ...
    reset_password_url = (
        f"http://127.0.0.1/accounts/password-reset/request?token="
        f"{reset_token.token}&emai={data.email}l"
    )
    background_tasks.add_task(
        email_sender.send_password_reset_email, str(data.email), reset_password_url
    )
...
async def reset_password(
    ...
    background_tasks: BackgroundTasks,  # <-- на початок
    db: AsyncSession = Depends(get_db),
    email_sender: EmailSenderInterface = Depends(get_accounts_email_notificator),
) -> MessageResponseSchema:
    """
    ...
    login_url = "http://127.0.0.1/accounts/login/"
    background_tasks.add_task(
        email_sender.send_password_reset_complete_email, str(data.email), login_url
    )
